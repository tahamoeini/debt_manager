...
Map<String, int> computeSpendingByCategory(
  ...
  for (final inst in installments) {
    ...
    final actual = inst['actual_paid_amount'] is int ? inst['actual_paid_amount'] as int : (inst['amount'] is int ? inst['amount'] as int : int.tryParse(inst['amount'].toString()) ?? 0);
...
List<Map<String, dynamic>> computeProjectDebtPayoff(
  ...
  final loanId = loan['id'] is int ? loan['id'] as int : int.tryParse(loan['id'].toString()) ?? -1;

  final insts = installments.where((i) => (i['loan_id'] is int ? i['loan_id'] as int : int.tryParse(i['loan_id'].toString()) ?? -1) == loanId).toList();
  insts.sort((a, b) => (a['due_date_jalali'] as String).compareTo(b['due_date_jalali'] as String));

  var balance = 0;
  for (final inst in insts) {
    final status = inst['status'] as String? ?? '';
    if (status != 'paid') balance += (inst['amount'] as int? ?? int.tryParse(inst['amount'].toString()) ?? 0);
  }
  ...
  for (final inst in insts) {
    ...
    var payment = inst['amount'] as int? ?? int.tryParse(inst['amount'].toString()) ?? 0;
