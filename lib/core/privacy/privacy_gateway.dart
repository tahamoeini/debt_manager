...
class PrivacyGateway {
  ...
  Future<void> importJsonString(String jsonStr) async {
    ...
    for (final it in installments) {
      ...
      final oldLoanId = instMap['loan_id'] is int
          ...
          : int.tryParse(instMap['loan_id'].toString()) ?? 0;
  ...
  Future<void> audit(String action, {String? details}) async {
    try {
      final dir = Directory
          .systemTemp; // lightweight local-only logging; adjust as needed
      final f = File('${dir.path}/debt_manager_audit.log');
      final entry =
          '${DateTime.now().toIso8601String()} | $action | ${details ?? ''}\n';
